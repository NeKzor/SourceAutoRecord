...
void Crosshair::Draw()
{
    ...
    surface->DrawRect(c, x1_end, yCenter - thickness, x1_start, yCenter + thickness + 1); //Left
    ...
    if (cl_crosshairdot.GetBool()) {
        surface->DrawRect(c, xCenter - thickness, yCenter - thickness, xCenter + 1 + thickness, yCenter + 1 + thickness); //Dot
    }
